...
class CloudSolrServersSpec extends FunSpec with Matchers {
  ...
  describe("CloudSolrServers") {
    ...
    it("should compute state changes correctly") {
      ...
      val events = CloudSolrServers.diff(
        oldState = Map(
          ...
          "col2" -> Seq(
            shardReplica("h20", Enabled, isLeader = true),
            shardReplica("h21", Enabled, isLeader = false),
            shardReplica("h22", Disabled, isLeader = false)
          )
        ),
        newState = Map(
          "col2" -> Seq(
            shardReplica("h21", Enabled, isLeader = true),
            shardReplica("h22", Enabled, isLeader = false),
            shardReplica("h23", Enabled, isLeader = false)
          ),
          "col3" -> Seq(shardReplica("h30", Enabled, isLeader = false))
        )
      )

      implicit val solrServerOrd: Ordering[SolrServer] =
        Ordering[String].on[SolrServer](s => s.baseUrl)
    ...
    it("should read all servers from ClusterState with multiple shards") {
      ...
      val bytes =
        Files.readAllBytes(Paths.get(this.getClass.getResource("/cluster_status.json").toURI))
      val cs = ClusterState.load(1, bytes, Set("server1:8983_solr").asJava)

      val collectionToServers = CloudSolrServers.getCollections(cs)
      collectionToServers("my-collection").servers should contain allOf (shardReplica(
